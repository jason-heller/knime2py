...
Overview
----------------------------
This module emits Python code and related artifacts from KNIME nodes,
integrating into the knime2py generator pipeline.

Runtime Behavior
----------------------------
Inputs:
The generated code reads input DataFrames or context keys specified in the
node's input lines.

Outputs:
Outputs are written to the `context` dictionary, mapping node IDs and ports
to DataFrames.

Key algorithms may include mappings to sklearn or imbalanced-learn equivalents,
with specific rules for column selection and handling of class imbalances.

Edge Cases
----------------------------
The code implements safeguards against empty or constant columns, NaNs, and
class imbalances, ensuring robust behavior in various scenarios.

Generated Code Dependencies
----------------------------
The generated code requires external libraries such as pandas, numpy, sklearn,
and imbalanced-learn. These dependencies are required by the generated code,
not by this module.

Usage
----------------------------
This module is typically invoked by the knime2py emitter, which processes
upstream nodes to generate corresponding Python code. An example of expected
context access might look like:
```python
...
Node Identity
----------------------------
Handles nodes identified by KNIME factory IDs, which are managed through the
`get_handlers` function. Special flags include `LOOP`, which indicates the
start or end of a loop in the workflow.

Configuration
----------------------------
The module uses the `NodeBlock` dataclass to represent node information.
...
The `parse_*` functions extract configuration values from `settings.xml` using
paths and xpaths, with appropriate fallbacks.

Limitations
----------------------------
Certain options available in KNIME may not be fully supported or approximated
